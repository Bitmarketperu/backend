...
router.post('/', async (req, res) => {
    ...
    const { wallet, id, amountSend, amountReceive, moneySend, moneyReceive, network, status, bankAdmin, bank, reciveUser, reciveNetwork, reciveMethod, payMethod, origen, politico } = req.body;
   
    try {
        if(!wallet || !id || !amountSend || !amountReceive || !moneySend || !moneyReceive || !network ||  !status || !bankAdmin || !bank || !reciveUser || !reciveNetwork || !reciveMethod || !payMethod) throw "data invalida";
        const responseController = await controller.addTransaction(wallet, id, amountSend, amountReceive, moneySend, moneyReceive, network, status, bankAdmin, bank, reciveUser, reciveNetwork, reciveMethod, payMethod, origen, politico );
        response.success(req, res, responseController, 200);
    } catch (error) {
        console.log(error)
        response.error(req, res, error, 401);
