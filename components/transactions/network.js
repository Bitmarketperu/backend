...
router.post('/', async (req, res) => {
    const { wallet, id, payMethod, bank, bankAdmin, amountSend, amountReceive, moneySend, moneyReceive, network, status } = req.body;
    try {
        if(!wallet || !id || !payMethod || !bank || !bankAdmin || !amountSend || !amountReceive || !moneySend || !moneyReceive || !network || !status) throw "data invalida";
        const responseController = await controller.addTransaction( wallet, id, payMethod, bank, bankAdmin, amountSend, amountReceive, moneySend, moneyReceive, network, status );
        response.success(req, res, responseController, 200);
    } catch (error) {
        console.log(error)
        response.error(req, res, error, 401);
